...
type UserType = (IUser & { userType: 'superadmin' }) | (IUser & { userType: 'admin' });
...
export function setupAuth(app: Express) {
    ...
    new LocalStrategy(
      ...
      async (email, password, done) => {
        try {
          ...
          const superAdmin = await mongoStorage.getSuperAdminByEmail(email);
          ...
          const admin = await mongoStorage.getAdminByEmail(email);
  ...
  passport.serializeUser((user: UserType, done) => {
    console.log(`Serializing ${user.userType}:`, user._id);
    // Store both the ID and user type in the session
    done(null, { id: user._id.toString(), userType: user.userType });
  });

  // Deserialize user from session
  passport.deserializeUser(async (serialized: { id: string, userType: string }, done) => {
    try {
      console.log(`Deserializing ${serialized.userType} id:`, serialized.id);
      
      if (serialized.userType === 'superadmin') {
        const superAdmin = await mongoStorage.getSuperAdmin(serialized.id);
        if (!superAdmin) {
        ...
        const admin = await mongoStorage.getAdmin(serialized.id);
        if (!admin) {
